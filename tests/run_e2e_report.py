...
TIERS = [
    ...
    },
    {
        ...
        "path": (
            "tests/test_adversarial.py::TestAdversarialSinkPolicy"
            "::test_model_exfiltration_attempt_blocked "
            "tests/test_adversarial.py::TestAdversarialSinkPolicy"
            "::test_model_subtle_redirect_blocked"
        ),
        "requires_model": True,
    },
]
...
def generate_markdown(env: dict, tiers: list[TierResult]) -> str:
    ...
    lines.append("")
    lines.append(
        f"**Status:** {status} | **Tests:** {total_passed}/{total_tests} passed "
    ...
    for tier in tiers:
        ...
        lines.append("")
        lines.append(
            f"**{tier_status}** | {tier.passed}/{tier_total} passed | {tier.duration:.1f}s"
        )
        lines.append("")
    ...
    lines.append("---")
    lines.append("*Report generated by `tests/run_e2e_report.py`*")
    lines.append("")
...
def main():
    ...
    if merge_only:
        ...
        for tier in TIERS:
            ...
            print(f"  {status}: {result.passed}/{total} passed in {result.duration:.1f}s")
                ...
                try:
                    ...
                    for testcase in root.iter("testcase"):
                        ...
                        if failure is not None:
                            ...
                            result.tests.append(
                                TestResult(name=test_name, passed=True, duration=test_time)
                            )
                except ET.ParseError:
